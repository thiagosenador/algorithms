...
def largest_product_grid(size):
    max_product = 0
    product = 0

    for i in range(0, len(GRID)):
        for j in range(0, len(GRID[i])):
            product = to_right(i, j, size)

            if product > max_product:
                max_product = product

            product = to_down(i, j, size)

            if product > max_product:
                max_product = product

            product = to_diagonal_right(i, j, size)

            if product > max_product:
                max_product = product

            product = to_diagonal_left(i, j, size)
...
def to_diagonal_right(row, column, size):
    if column + size > len(GRID[row]) or row + size > len(GRID):
        return 0
...
def to_diagonal_left(row, column, size):
    if column + 1 - size < 0 or row + size > len(GRID):
        return 0
